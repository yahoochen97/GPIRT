...
MODELS = c("gpirt", "SEirt" ,"doirt","DSEM", "ggum" ,"graded", "sequential", "gpcm")
...
for(h in 1:horizon) {
  ...
  for(k in 2:length(MODELS)){
    ...
    if(h_>=0){
      tmp = t.test(ttest_acc[,h_+1,1], ttest_acc[,h_+1,k])
      results[nrow(results)+1,] = c(h_, "acc", tmp[["p.value"]],
                mean(ttest_acc[,h_+1,1])-mean(ttest_acc[,h_+1,k]), MODEL_COMPARE)
      # -tmp[['estimate']][[2]]
      tmp = t.test(ttest_lls[,h_+1,1], ttest_lls[,h_+1,k])
      results[nrow(results)+1,] = c(h_, "ll", tmp[["p.value"]],
                mean(ttest_lls[,h_+1,1])-mean(ttest_lls[,h_+1,k]),MODEL_COMPARE)
      
      tmp = t.test(ttest_sds[,h_+1,1], ttest_sds[,h_+1,k])
      results[nrow(results)+1,] = c(h_, "sd", tmp[["p.value"]],
                 mean(ttest_sds[,h_+1,1])-mean(ttest_sds[,h_+1,k]),MODEL_COMPARE)
    }
  }
